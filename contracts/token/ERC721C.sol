    ...
    IERC721C
{
    ...
    constructor(
        ...
        _quarkAddress = address(
            new Quark(
                name_,
                symbol_,
                layerCount_,
                uint256(layerCount_) * collectionSize_
            )
        );
        require(
            IComposableFactory(composableFactoryAddress_).addQToCAddressMapping(
                _quarkAddress,
                address(this)
            ),
            "Join the Pool Failed."
        );
    ...
    function burn(address tokenOwner, uint256 tokenId) external override {
        require(
            msg.sender == composableFactoryAddress,
    ...
    function tokenByIndex(uint256 index)
        ...
        returns (uint256)
    {
        require(_exists(index), "index not exits");
        return index;
    }

    function poolMint(address to) external override returns (uint256) {
        require(
            composableFactoryAddress == msg.sender,
            "Pool Mint need a Binding Pool"
        );
        require(to != address(0), "mint to the zero address");
        // We know if the first token in the batch doesn't exist, the other ones don't as well, because of serial ordering.
        require(!_exists(_currentIndex), "token already minted");
    ...
    function safeMint(address to) internal {
        ...
        require(
            composableFactoryAddress != address(0),
            "must join the pool before mint"
        );
        // mint Quarks, add mapping
        uint256 startQuarkIndex = Quark(_quarkAddress).cMint(
            composableFactoryAddress,
            _layerCount
        );
        ...
        IComposableFactory(composableFactoryAddress).addCIdToQuarksMapping(
            _quarkAddress,
            _currentIndex,
            tokenIds
        );
